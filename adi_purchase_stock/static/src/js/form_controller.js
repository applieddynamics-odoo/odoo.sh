odoo.define("adi_purchase_stock.FormController", function (require) {
  ...
  FormController.include({
    _saveRecord(recordId, options) {
      if (
        this.modelName == "purchase.order" &&
        !(options && options.stayInEdit)
      ) {
        ...
        return this._rpc({
          ...
          if (should_warn) {
            return confirm(
              self,
              "The selected vendor has an on-time delivery rate below the current threshold. Please ensure there are no better alternatives for this order. Do you want to continue?",
              {
