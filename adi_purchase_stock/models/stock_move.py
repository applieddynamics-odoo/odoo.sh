    ...
    def _action_done(self):
        ...
        todo_moves = self.mapped('move_lines').filtered(
            lambda self: self.state in [
                'draft', 'waiting', 'partially_available', 'assigned',
                'confirmed'
            ])
        for picking in self:
            if picking.owner_id:
        ...
        done_incoming_moves = self.filtered(lambda p: p.picking_type_id.code ==
                                            'incoming').move_lines.filtered(
...
class StockMove(models.Model):
    ...
    def _create_account_move_line(self, credit_account_id, debit_account_id,
        ...
        move_lines = self._prepare_account_move_line(qty, cost,
                                                     credit_account_id,
                                                     debit_account_id,
                                                     description)
        if move_lines:
            ...
            if purchase_order:
                ...
                date = ctx.get('force_period_date',
