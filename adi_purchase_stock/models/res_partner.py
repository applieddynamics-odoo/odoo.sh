...
class ResPartner(models.Model):
    ...
    def _compute_on_time_rate(self):
        order_lines = self.env['purchase.order.line'].search([
            ...
            lambda l: l.product_id.sudo().product_tmpl_id.type != 'service')
        ...
        moves = self.env['stock.move'].search([
            ...
            # custom code started
            ('state', '=', 'done')
        ]).filtered(lambda m: m.date.astimezone(tz=tzInfo).date() <= m.
                    purchase_line_id.date_planned.astimezone(tz=tzInfo).date())
