...
class StockRule(models.Model):
    ...
    def _update_purchase_order_line(self, product_id, product_qty, product_uom,
        ...
        rerun_quantity_data = self.calculate_qty_for_rerun_move(
            move_from_procurement, requested_quantity)
        if bool(rerun_quantity_data):
            res['product_qty'] = requested_quantity - rerun_quantity_data.get(
                'previous_quantity') + qty_from_po
    ...
    def filter_procurements_on_forecast_qty(self, procurements):
        ...
        for procurement, rule in procurements:
            ...
            requested_quantity = procurement.product_uom._compute_quantity(
            ...
            if move_from_procurement:
                ...
                if quantity_already_processed_move:
                    if requested_quantity > quantity_already_processed_move:
                        procurements_to_be_considered.append(
                            [procurement, rule])
                else:
                    needed_quantity = requested_quantity - product_virtual_quantity if requested_quantity > product_virtual_quantity else 0
                    if needed_quantity:
    ...
    def calculate_qty_for_rerun_move(self, stock_move, requested_quantity):
        ...
        if len(stock_move) <= 1:
            ...
            } if bool(difference_amount) else {}
            return rerun_data
        else:
            ...
            rerun_data = {
                'difference_amount': total_difference_amount,
                'previous_quantity': total_previous_quantity
            } if bool(total_difference_amount) else {}
            return rerun_data
